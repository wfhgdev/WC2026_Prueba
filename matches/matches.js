// ============================================
// PÁGINA: Partidos y Resultados (i18n)
// ============================================

// ---------- Helper: filtros activos ----------
...
async function renderMatches(status = 'ALL') {
  ...
  try {
    ...
    matches.forEach(match => {
      ...
      if (isLive) {
        // Live card has gradient top bar
      ...
      innerHTML += `</div></div>`; // close teams row, close p-5

      // Bottom bar: venue (solo si el dato existe)
      if (match.venue) {
        innerHTML += `
